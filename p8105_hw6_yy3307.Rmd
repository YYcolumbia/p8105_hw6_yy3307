...
The first step is to load and clean the data for regression analysis, we need to convert numeric to factor and check for missing data. I would change the numeric to the corresponding expression first using `recode`, so that I can more directly recognize the representation. Then we can use `as.factor` to change them into factors.
...
```{r}
bwt_model =
  lm(formula = bwt ~ babysex + bhead + blength + delwt + fincome + gaweeks + mheight + mrace + parity + ppwt + smoken, data = birthweight_raw) %>% 
  broom::tidy() %>% 
  knitr::kable(digits = 3)
...
After getting the fitted model for birthweight, we can make a plot of model residuals against fitted values.
